...
import com.niccher.mpesa_analyzer_app.fragments.History.SummaryFragment
import com.niccher.mpesa_analyzer_app.models.SummariesModel
import java.text.SimpleDateFormat
import java.util.*

class HistoryAdapter(
    private val summariesList: ArrayList<SummariesModel>,
    ...
    class MyViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        val part_volume: TextView = itemView.findViewById(R.id.part_txt_volume)
        val part_senders: TextView = itemView.findViewById(R.id.part_txt_senders)
    }
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        val isPending = summary.summary_Count > 0 && (summary.category_breakdown == null || summary.category_breakdown.isEmpty())
        ...
        holder.part_sent.text = if (isPending) "--" else "Ksh ${fmt.format(summary.summary_Sent)}"
        holder.part_unknown.text = if (isPending) "--" else summary.summary_Unknown.toString()
        holder.part_receive.text = if (isPending) "--" else "Ksh ${fmt.format(summary.summary_Received)}"
        holder.part_count.text = "${summary.summary_Count} SMS"

        if (isPending) {
            holder.part_volume.text = "Volume: Processing..."
            holder.part_senders.text = "Analysis Pending"
        } else {
            holder.part_volume.text = "Volume: Ksh ${fmt.format(summary.total_amount)}"
            holder.part_senders.text = "${summary.finance_senders} Senders"
        }
        ...
        holder.part_frame.setOnClickListener {
            ...
            val navController = androidx.navigation.Navigation.findNavController(activity, R.id.nav_host_fragment_activity_bottom)

            val bundle = Bundle().apply {
                putString("summary_loot_name", summary.summary_Loot_Uuid)
                if (summary.category_breakdown != null) {
                    putString("category_breakdown", Gson().toJson(summary.category_breakdown))
                }
            }
            navController.navigate(R.id.navi_summary, bundle)
        }
    }

    private fun buildCategoryBadges(container: LinearLayout, summary: SummariesModel) {
        container.removeAllViews()
        val density = context.resources.displayMetrics.density
        val breakdown = summary.category_breakdown

        if (breakdown == null || breakdown.isEmpty()) {
            val statusView = TextView(context).apply {
                text = "● Analysis In Progress..."
                setTextColor(ContextCompat.getColor(context, R.color.semantic_warning))
                textSize = 12f
                setTypeface(null, android.graphics.Typeface.BOLD)
                setPadding((8 * density).toInt(), (4 * density).toInt(), (8 * density).toInt(), (4 * density).toInt())
                background = ContextCompat.getDrawable(context, R.drawable.bg_rounded_label)?.apply {
                    setTint(ContextCompat.getColor(context, R.color.bg_primary_faint))
                }
            }
