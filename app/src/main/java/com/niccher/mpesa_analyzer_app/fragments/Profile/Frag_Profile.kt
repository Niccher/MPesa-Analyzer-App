...
class Frag_Profile : Fragment() {
    ...
    private fun loadUserProfile() {
        ...
        val lootPrefs = requireContext().getSharedPreferences(Konstants.SHARED_LOOT_COUNT, Context.MODE_PRIVATE)
    ...
    private fun fetchUserInfoFromServer(userId: String) {
        val jsonAuthUser = ServiceGenerators.createService(JsonAuthUser::class.java, requireContext())
        jsonAuthUser.getUserInfo(userId).enqueue(object : Callback<Mod_User_Info> {
            override fun onResponse(call: Call<Mod_User_Info>, response: Response<Mod_User_Info>) {
            ...
            override fun onFailure(call: Call<Mod_User_Info>, t: Throwable) {
